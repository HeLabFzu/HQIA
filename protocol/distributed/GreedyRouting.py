...
"""
the principle of Greedy_algorithm is that Starting from src-host, each node acts independently, and each node selects the node closest to dst-host as its next hop. 
Therefore, we need to store a routing table in each node to shortest hops from its neighboring nodes to dst-host, implementing the Greedy algorithm.
"""
...
class NetworkRoutingTable:
    def __init__(self,network):
        ...
        self.network_routing_table.append(NodeRoutingTable(Repeater_B, [User_A,Repeater_A,Repeater_C,Repeater_E,Repeater_F], [5,5,4,3,3]))
...
def Greedy(network,src_host,dst_host):
    nrt = NetworkRoutingTable(network)
    path_temp = [src_host]

    while path_temp[len(path_temp)-1] != dst_host:
        present_node = path_temp[len(path_temp)-1]
    ...
    for i in range(len(path_temp)-1):
        ...
        controller = dt.findcontroller(path_temp[i].name,path_temp[i+1].name)
        path.append(controller)
        if i == len(path_temp)-2:
            path.append(path_temp[i+1])
    FinalResult = []
