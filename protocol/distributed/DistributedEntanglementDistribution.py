...
class DistributedEntanglementDistribution(NodeProtocol):
    ...
    def __init__(self, node, role, store_mem_pos=None, qsource_name=None, start_expression=None, input_mem_pos=0, name=None):
        if role.lower() not in ["generator", "receiver"]:
            raise ValueError
        self._is_generator = role.lower() == "generator"
    ...
    def run(self):
        ...
        if self._is_generator:
            ...
            ### wait 1 ns, until the gennerator mempos receive its own entangled qubit ###
            yield self.await_timer(1)
            for pos in range(4):
                if not self.node.qmemory.mem_positions[pos].is_empty:
                    self.node.qmemory.execute_instruction(INSTR_SWAP, [pos, self._store_mem_pos])
                    break
            ...
            print("Node {} mem {} received qubit".format(self.name, self._store_mem_pos))
    def check(self):
        if not self._is_generator and self.get_signal_result(Signals.SUCCESS) is None:
            return False
        else:
            return True
